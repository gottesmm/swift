...
perspective, this sort of API verbotten and leads to bugs, crashes, and security
...
This ensures statically that any uses of the address that are not escaped to be
an object type (e.x.: `address_to_pointer`_) are within the guaranteed pointers
scope where the guaranteed value is statically known to be live. As a result, in
SIL it is impossible to express such a bug in compiler generated code. As an
example, consider the following unsafe interior pointer SIL::

    // Today SIL restricts interior pointer instructions to only have operands
    // with guaranteed ownership. The author thinks it could be implemented
...
which are viewed as outside of the ownership system since they have .none
ownership.
...
In the second example, we show how by forcing interior pointer instructions to
only have guaranteed operands and propagating up address uses trasitively to be
uses of the underlying object, ``%0`` in the example.
