...
struct PartitionOpEvaluator {
  ...
  /// inst.
  std::function<bool(Element elt, SILInstruction *inst)>
      isClosureCapturedCallback = nullptr;
  ...
  bool isClosureCaptured(Element elt, SILInstruction *inst) const {
    return bool(isClosureCapturedCallback) &&
           isClosureCapturedCallback(elt, inst);
  }
  ...
  void apply(const PartitionOp &op) const {
    ...
    case PartitionOpKind::Transfer: {
      ...
      bool isClosureCapturedElt =
          isClosureCaptured(op.getOpArgs()[0], op.getSourceInst());
      ...
      for (const auto &pair : p.elementToRegionMap) {
        ...
        isClosureCapturedElt |=
            isClosureCaptured(pair.first, op.getSourceInst());
      }
