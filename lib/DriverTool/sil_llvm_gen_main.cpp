...
struct SILLLVMGenOptions {
  ...
  llvm::cl::opt<bool> EnableLibraryEvolution = llvm::cl::opt<bool>(
...
/// A helper to check for specific options that are LLVM options.
///
/// We need to use this to check for specific LLVM options that we would in a
/// normal compiler invocation recognize as -Xllvm flags and pass through to
/// LLVM directly but also stash them so we know that we have them. In
/// sil-llvm-gen, we do not have that ability since we use LLVM opt directly so
/// we are unable to tell if this option is actually passed through since any
/// option could be an LLVM option and LLVM options are not prefixed with
/// -Xllvm.
static bool checkForOption(ArrayRef<const char *> argv,
                           StringRef optionToCheck) {
  for (auto arg : argv) {
    if (optionToCheck == arg)
      return true;
  }
  return false;
}
...
int sil_llvm_gen_main(ArrayRef<const char *> argv, void *MainAddr) {
  ...
  Opts.HasAArch64TBI = checkForOption(argv, "-aarch64-use-tbi");
