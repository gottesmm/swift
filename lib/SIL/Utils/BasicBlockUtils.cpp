...
void JointPostDominanceSetComputer::findJointPostDominatingSet(
    SILBasicBlock *dominatingBlock, ArrayRef<SILBasicBlock *> partialPostDomSet,
  ...
  // If our post dom set is empty, assert.
  assert(partialPostDomSet.size() != 0);

  // If we have a partial post dom set with a single block and that block is
  // dominating block, then we return success since a block post-doms its self
  // so it is already complete.
  if (partialPostDomSet.size() == 1) {
    if (dominatingBlock == partialPostDomSet[0])
      return;
  }
  ...
  // Otherwise, we need to complete our post dominating set. We do this by
  ...
  while (!worklist.empty()) {
    ...
    for (auto *predBlock : block->getPredecessorBlocks()) {
      if (initialBlocks.count(predBlock)) {
        foundLoopBlockCallback(predBlock);
      }
