...
ManagedValue
SILGenBuilder::createOwnedMoveOnlyToCopyableValue(SILLocation loc,
                                                  ManagedValue value) {
  assert(value.isPlusOne(SGF) && "Argument must be at +1!");
  CleanupCloner cloner(*this, value);
  auto *mdi = createOwnedMoveOnlyToCopyableValue(
      loc, value.forward(getSILGenFunction()));
  return cloner.clone(mdi);
}

ManagedValue
SILGenBuilder::createGuaranteedMoveOnlyToCopyableValue(SILLocation loc,
                                                       ManagedValue value) {
  auto *mdi = createGuaranteedMoveOnlyToCopyableValue(loc, value.getValue());
