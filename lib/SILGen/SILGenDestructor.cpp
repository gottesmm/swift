...
void SILGenFunction::emitDestroyingDestructor(DestructorDecl *dd) {
  ...
  ManagedValue selfValue = ManagedValue::forUnmanaged(emitSelfDecl(dd->getImplicitSelfDecl()));
  ...
  if (cd->hasSuperclass()) {
    ...
    ManagedValue baseSelf = B.createUpcast(cleanupLoc, selfValue, baseSILTy);
    ManagedValue dtorValue;
    SILType dtorTy;
    ArrayRef<Substitution> subs
      = superclassTy->gatherAllSubstitutions(SGM.M.getSwiftModule(), nullptr);
    std::tie(dtorValue, dtorTy, subs)
      = emitSiblingMethodRef(cleanupLoc, baseSelf.getValue(), dtorConstant, subs);
    SILValue superReturnedSelf =
      B.createApply(cleanupLoc, dtorValue.forward(*this), dtorTy, objectPtrTy,
                    subs, baseSelf.forward(*this));
    resultSelfValue = emitManagedRValueWithCleanup(superReturnedSelf);
  } else {
    resultSelfValue = selfValue.copyUnmanaged(*this, cleanupLoc);
  }

  {
    FullExpr ClassMemberDestructorScope(Cleanups, cleanupLoc);
    ManagedValue borrowedResultSelfValue = resultSelfValue.borrow(*this, cleanupLoc);
    SILValue castBorrowedValue = B.createUncheckedRefCast(cleanupLoc,
                                                          borrowedResultSelfValue.getUnmanagedValue(),
                                                          classTy);

    // Release our members.
    emitClassMemberDestruction(castBorrowedValue, cd, cleanupLoc);
  }

  resultSelfValue = B.createUncheckedRefCast(cleanupLoc, resultSelfValue,
                                             objectPtrTy);
  B.createReturn(returnLoc, resultSelfValue.forward(*this));
}
...
void SILGenFunction::emitDeallocatingDestructor(DestructorDecl *dd) {
  ...
  SILType classTy;
  {
    ...
    ManagedValue selfValue =
      emitManagedRValueWithCleanup(emitSelfDecl(dd->getImplicitSelfDecl()));

    // Form a reference to the destroying destructor.
    SILDeclRef dtorConstant(dd, SILDeclRef::Kind::Destroyer);
    classTy = selfValue.getType();
    ManagedValue dtorValue;
    SILType dtorTy;
    ArrayRef<Substitution> subs = classTy.gatherAllSubstitutions(SGM.M);
    std::tie(dtorValue, dtorTy, subs)
      = emitSiblingMethodRef(loc, selfValue.getValue(), dtorConstant, subs);

    // Call the destroying destructor.
    ManagedValue borrowedSelf = selfValue.borrow(*this, loc);
    SILType objectPtrTy = SILType::getNativeObjectType(F.getASTContext());
    selfValueForDealloc =
      B.createApply(loc, dtorValue.forward(*this),
                    dtorTy, objectPtrTy, subs, borrowedSelf.getUnmanagedValue());
  }

  // Deallocate the object after casting back to classTy.
  selfValueForDealloc =
          B.createUncheckedRefCast(loc, selfValueForDealloc,
                                   classTy);
  B.createDeallocRef(loc, selfValueForDealloc, false);
