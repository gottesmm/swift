...
ManagedValue SILGenFunction::emitManagedRValueWithCleanup(SILValue v,
                                               const TypeLowering &lowering) {
  assert(lowering.getLoweredType().getObjectType() == v->getType().getObjectType());
  if (lowering.isTrivial())
    return ManagedValue::forUnmanaged(v);
...
void SILGenFunction::emitBindOptional(SILLocation loc,
  ...
  auto hasValue = emitDoesOptionalHaveValue(loc,
                                            optionalAddrOrValue.getValue());
...
public:
  ...
  RValue get(SILGenFunction &gen, SILLocation loc,
    ...
    SILValue unowned = gen.B.createRefToUnmanaged(loc, loadedBase.getUnmanagedValue(),
                                                  unownedType);
