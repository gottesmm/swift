  ...
  SILValue updateArgumentValueForBinding(ManagedValue argrv, SILLocation loc,
    ...
    if (!isNoImplicitCopy) {
      ...
      if (value->getOwnershipKind() == OwnershipKind::Owned) {
        auto *fArg = cast<SILFunctionArgument>(value);
        auto *varDecl = cast<VarDecl>(fArg->getDecl());
        SILDebugVariable dbgVar(varDecl->isLet(), fArg->getIndex());
        auto *box = SGF.B.createAllocBox(loc, value->getType(), dbgVar);
