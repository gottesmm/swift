...
#include "swift/SILOptimizer/Utils/OwnershipUtils.h"
...
#include "swift/AST/ReferenceStorage.def"
    ...
    SILValue visitUncheckedOwnershipConversionInst(
...
SILValue InstSimplifier::visitPointerToAddressInst(PointerToAddressInst *PTAI) {
  // To support this with ownership, we need to do something more complex that
  // lifetime extends a borrowed value to maintain safety of the address if the
  // address is an interior pointer.
...
SILValue InstSimplifier::visitUncheckedOwnershipConversionInst(
    UncheckedOwnershipConversionInst *uoci) {
  auto op = uoci->getOperand();
  if (op.getOwnershipKind().isCompatibleWith(
...
SILBasicBlock::iterator swift::replaceAllSimplifiedUsesAndErase(
  ...
  if (svi->getFunction()->hasOwnership()) {
    ...
    OwnershipFixupContext ctx{eraseNotify, newInstNotify, *deadEndBlocks,
                              computer};
    return ctx.replaceAllUsesFixingOwnership(svi, result);
  }
