...
void EagerDispatch::emitDispatchTo(SILFunction *NewFunc) {
  ...
  if (NewFunc->isNoReturnFunction(Builder.getTypeExpansionContext()) ||
    ...
    SILValue CastResult;
    if (GenResultTy.isTrivial(Builder.getFunction())) {
      CastResult =
          Builder.createUncheckedTrivialBitCast(Loc, Result, GenResultTy);
    } else {
      CastResult = Builder.createUncheckedValueCast(Loc, Result, GenResultTy);
    }
...
SILValue EagerDispatch::emitArgumentCast(CanSILFunctionType CalleeSubstFnTy,
  ...
  if (CastTy.isTrivial(Builder.getFunction()))
    return Builder.createUncheckedTrivialBitCast(Loc, OrigArg, CastTy);

  return Builder.createUncheckedValueCast(Loc, OrigArg, CastTy);
}
