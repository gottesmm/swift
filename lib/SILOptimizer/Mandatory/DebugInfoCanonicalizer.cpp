...
struct DebugInfoCanonicalizer {
  ...
  SmallVector<SILInstruction *, 32> debugInstToBreakBlocksAt;
    ...
    do {
      ...
      for (auto &pred : domBlockState.debugValues) {
        ...
        if (auto *inst = insertPt.dyn_cast<SILInstruction *>()) {
          debugInstToBreakBlocksAt.push_back(
...
bool DebugInfoCanonicalizer::process() {
  ...
  // Break after any debug_value we inserted to work around CodeGen bugs.
  if (!debugInstToBreakBlocksAt.empty()) {
    SILBuilderContext ctx(fn->getModule());
    do {
      auto *next = debugInstToBreakBlocksAt.pop_back_val();
      splitBasicBlockAndBranch(ctx, next->getNextInstruction(), nullptr,
                               nullptr);
    } while (!debugInstToBreakBlocksAt.empty());
  }
