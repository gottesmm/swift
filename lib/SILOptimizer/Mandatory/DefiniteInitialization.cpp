...
#include "swift/AST/ReferenceStorage.def"
  ...
  if (auto *AI = dyn_cast<AssignInst>(Inst)) {
    ...
    if (InitKind == IsInitialization) {
      if (auto *mmci = dyn_cast<MarkMustCheckInst>(stripAccessMarkers(AI->getDest()))) {
        if (mmci->getCheckKind() == MarkMustCheckInst::CheckKind::AssignableButNotConsumable &&
            isa<RefElementAddrInst>(mmci->getOperand())) {
          mmci->setCheckKind(MarkMustCheckInst::CheckKind::InitableButNotConsumable);
        }
      }
    }
