    ...
    if (boxType->getLayout()->isMutable()) {
      ...
      diag = diag::
          sil_moveonlychecker_notconsumable_but_assignable_was_consumed_escaping_let;
                 ...
                 dyn_cast<SILFunctionArgument>(markedValue->getOperand())) {
    ...
    if (!cast<VarDecl>(fArg->getDecl())->isLet()) {
      ...
      diag = diag::
          sil_moveonlychecker_notconsumable_but_assignable_was_consumed_escaping_let;
                 ...
                 dyn_cast<BeginAccessInst>(markedValue->getOperand())) {
    SILValue value = access->getOperand();
    if (auto *pbi = dyn_cast<ProjectBoxInst>(value))
      value = pbi->getOperand();
    if (auto *fArg = dyn_cast<SILFunctionArgument>(value)) {
      // If allocbox_to_stack eliminated the box for an escaping var, we may
      // have a mark_must_check on a SILFunctionArgument.
      assert(fArg->isClosureCapture());
      if (!cast<VarDecl>(fArg->getDecl())->isLet()) {
        diag = diag::
            sil_moveonlychecker_notconsumable_but_assignable_was_consumed_escaping_var;
