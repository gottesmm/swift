...
#define RAUW_ALWAYS(CLS)                                                       \
  ...
  RAUW_ALWAYS(MoveOnlyToCopyableValue)
  RAUW_ALWAYS(CopyableToMoveOnlyValue)
#undef RAUW_ALWAYS
  ...
  for (auto &bb : *fn) {
    ...
    if (&bb != &fn->front()) {
      for (auto *arg : bb.getArguments()) {
        if (arg->getType().isMoveOnly()) {
          if (arg->unsafelyEliminateMoveOnly()) {
    ...
    for (auto &ii : bb) {
      for (SILValue v : ii.getResults()) {
        if (v->getType().isMoveOnly()) {
          v->unsafelyEliminateMoveOnly();
