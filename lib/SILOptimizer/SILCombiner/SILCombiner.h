...
public:
  ...
  /// A version of \see SILCombiner#withBuilder that trades the need to specify
  /// the result ty at the use site with the ability to return an arbitrary
  /// result. So one would then write:
  ///
  /// auto *finalValue =
  ///   withBuilder<MyType *>(insertPt, [&](auto &b, auto l) {
  ///     b.createInst1(l, ...);
  ///     b.createInst2(l, ...);
  ///     b.createInst3(l, ...);
  ///     return b.createInst4(l, ...);
