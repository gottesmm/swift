...
static bool canPromoteAllocBox(AllocBoxInst *ABI,
  ...
  if (auto *User = findUnexpectedBoxUse(ABI,
    ...
    // Check if our underlying type has a "boxtostack.mustbeonstack"
    // attribute. If so, emit a diagnostic.
    if (auto decl = ABI->getAddressType().getNominalOrBoundGenericNominal()) {
