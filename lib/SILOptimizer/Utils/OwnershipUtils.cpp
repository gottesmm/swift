...
SILBasicBlock::iterator OwnershipRAUWUtility::perform() {
  assert(!isa<SILUndef>(newValue));
  assert(oldValue->getFunction()->hasOwnership());
#ifndef NDEBUG
  performExhaustiveOwnershipChecks(oldValue->getFunction());
#endif
  assert(
...
//===----------------------------------------------------------------------===//
//
// Top level entry point to RAUW code.
//
SILBasicBlock::iterator OwnershipFixupContext::replaceAllUsesFixingOwnership(
