...
func testTaskLocal(_ y: NSObject) async {
  ...
  // expected-ni-ns-note @-1 {{sending 'x' to @concurrent global function 'useValueConcurrently' risks causing data races between @concurrent code and code in the current task}}
  ...
  // expected-note @-1 {{sending 'x' to main actor-isolated global function 'useValueMainActor' risks causing data races between main actor-isolated code and code in the current task}}
}
