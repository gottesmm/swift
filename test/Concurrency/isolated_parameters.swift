...
func checkConformer(_ s: S, _ p: any P, _ ma: MyActor) async {
  ...
  // expected-note @-2 {{sending 'p' to actor-isolated instance method 'm(thing:)' risks causing data races between actor-isolated code and code in the current task}}
}
...
nonisolated func callFromNonisolated(ns: NotSendable) async {
  ...
  // expected-note @-2 {{sending 'ns' to actor-isolated global function 'optionalIsolated(_:to:)' risks causing data races between actor-isolated code and code in the current task}}
}
