...
final class NonSendable {
  ...
  func call() async {
    ...
    // expected-tns-note @-2 {{sending 'self' to main actor-isolated instance method 'update()' risks causing data races between main actor-isolated code and code in the current task}}
...
struct DowngradeForPreconcurrency {
  ...
  func createStream() -> AsyncStream<NonSendable> {
    AsyncStream<NonSendable> {
      self.x // expected-tns-ni-warning {{assigning '$return_value' to 'self.x.some' risks causing data races; 'self.x.some' is accessible to code in the current task; this is an error in the Swift 6 language mode}}
      // expected-tns-ni-note @-1 {{'$return_value' could become accessible to code in the current task}}
      // expected-tns-ni-ns-warning @-2 {{assigning '$return_value' to 'self.x.some' risks causing data races; 'self.x.some' is accessible to code in the current task; this is an error in the Swift 6 language mode}}
      // expected-tns-ni-ns-note @-3 {{'$return_value' could become accessible to code in the current task}}
