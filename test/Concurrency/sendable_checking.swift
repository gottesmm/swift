...
@available(SwiftStdlib 5.1, *)
  ...
  func bar(x : () -> ()) -> () {} // expected-warning {{pattern that the region-based isolation checker does not understand how to check}}
  ...
  func bar2<T>(x : T) -> () {} // expected-warning {{pattern that the region-based isolation checker does not understand how to check}}
  ...
  func bar3<T: Equatable>(x : T) -> () {} // expected-warning {{pattern that the region-based isolation checker does not understand how to check}}
...
final class NonSendable {
  ...
  func call() async {
    ...
    _ = await x // expected-warning {{pattern that the region-based isolation checker does not understand how to check}}
    // expected-warning@-1 {{non-Sendable type 'NonSendable' cannot be sent into main actor-isolated context in call to property 'x'}}

    _ = await self.x // expected-warning {{pattern that the region-based isolation checker does not understand how to check}}
      // expected-warning@-1 {{non-Sendable type 'NonSendable' cannot be sent into main actor-isolated context in call to property 'x'}}
  }
