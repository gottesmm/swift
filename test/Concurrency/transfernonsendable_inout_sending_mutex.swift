...
func testMutexWithLockAssignsToCapture() {
  ...
  let _ = m.withLock { (state: inout sending NonSendableKlass) -> NonSendableKlass in
    state = m2
    return state // expected-error {{'inout sending' parameter 'state' is accessible to code in the current task at end of function}}
    // expected-note @-1 {{'state' risks causing races in between code in the current task and caller uses since caller assumes value is not actor isolated}}
