...
func testConsumingUseAfterConsumeError(_ x: consuming Klass) async { // expected-error {{'x' used after consume}}
  ...
  // expected-note @-1 {{sending 'x' to main actor-isolated global function 'consumeTransferToMain' risks causing data races between main actor-isolated code and code in the current task}}
