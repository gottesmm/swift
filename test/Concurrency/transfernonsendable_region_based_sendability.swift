...
func test_arg_nonconsumable(a : A, ns_arg : NonSendable) async {
  ...
  // expected-note @-1 {{sending 'ns_arg' to actor-isolated instance method 'foo' risks causing data races between actor-isolated code and code in the current task}}
  // expected-complete-warning @-2 {{passing argument of non-Sendable type 'NonSendable' into actor-isolated context may introduce data races}}
...
class C_NonSendable {
  ...
  func bar(a : A) async {
    ...
    // expected-note @-1 {{sending 'captures_self' to actor-isolated instance method 'foo' risks causing data races between actor-isolated code and code in the current task}}
