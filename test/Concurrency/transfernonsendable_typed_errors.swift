// RUN: %target-swift-frontend -swift-version 6 -Xllvm -sil-regionbasedisolation-force-use-of-typed-errors=all -emit-sil -o /dev/null %s -verify -verify-additional-prefix ni- -target %target-swift-5.1-abi-triple
// RUN: %target-swift-frontend -swift-version 6 -Xllvm -sil-regionbasedisolation-force-use-of-typed-errors=all -emit-sil -o /dev/null %s -verify -verify-additional-prefix ni-ns- -target %target-swift-5.1-abi-triple -enable-upcoming-feature NonisolatedNonsendingByDefault
...
struct MainActorStruct {
  ...
  nonisolated init(assign val: NonSendableKlass) {
    self.x = val // expected-error {{assigning a value of type 'NonSendableKlass' to main actor-isolated value of type '()' risks causing data races}}
  ...
  nonisolated init(nonisolatedFunc val: NonSendableKlass) {
    ...
    self.y = NonSendableKlass()
    useNonSendable(self.x, val) // expected-error {{passing a value of type 'NonSendableKlass' and main actor-isolated value of type 'NonSendableKlass' as arguments to global function 'useNonSendable' risks causing data races}}
    // expected-note @-1 {{a value of type 'NonSendableKlass' could begin referencing a value of type 'NonSendableKlass' allowing concurrent access to a value of type 'NonSendableKlass' by main actor-isolated code and code in the current task}}
  }
}
