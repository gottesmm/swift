// RUN: %target-swift-frontend -swift-version 6 -Xllvm -sil-regionbasedisolation-force-use-of-typed-errors=dst-only -emit-sil -o /dev/null %s -verify -verify-additional-prefix ni- -target %target-swift-5.1-abi-triple
// RUN: %target-swift-frontend -swift-version 6 -Xllvm -sil-regionbasedisolation-force-use-of-typed-errors=dst-only -emit-sil -o /dev/null %s -verify -verify-additional-prefix ni-ns- -target %target-swift-5.1-abi-triple -enable-upcoming-feature NonisolatedNonsendingByDefault

// REQUIRES: concurrency
// REQUIRES: asserts
// REQUIRES: swift_feature_NonisolatedNonsendingByDefault

// This test verifies that -sil-regionbasedisolation-force-use-of-typed-errors=dst-only
// only forces type-based diagnostics for the dst side of cross-isolation merge
...
struct MainActorStruct {
  ...
  // Assign merge: src uses name, dst uses type (value)
  nonisolated init(assign val: NonSendableKlass) {
    self.x = val // expected-error {{assigning 'val' to main actor-isolated value of type '()' risks causing data races}}
    // expected-note @-1 {{'val' could become accessible to main actor-isolated code despite remaining accessible to code in the current task}}
    self.y = NonSendableKlass()
  }

  // NonisolatedFunction merge: src uses name, dst uses type (value)
  nonisolated init(nonisolatedFunc val: NonSendableKlass) {
    self.x = NonSendableKlass()
    self.y = NonSendableKlass()
    useNonSendable(self.x, val) // expected-error {{passing 'val' and main actor-isolated value of type 'NonSendableKlass' as arguments to global function 'useNonSendable' risks causing data races}}
    // expected-note @-1 {{a value of type 'NonSendableKlass' could begin referencing 'val' allowing concurrent access to 'val' by main actor-isolated code and code in the current task}}
  }
}
