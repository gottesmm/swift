...
// RUN: %empty-directory(%t)
// RUN: %swift-ide-test_plain -print-usrs -target %target-triple  %resource-dir-opt -module-cache-path %t  -I %S/Inputs -source-filename %s
...
// RUN: %swift-ide-test_plain -print-usrs -target %target-triple -module-cache-path %t %resource-dir-opt -I %S/Inputs -source-filename %s
