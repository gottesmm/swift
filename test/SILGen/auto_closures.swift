...
// CHECK-LABEL: sil hidden @$S13auto_closures05call_A8_closureyAA4BoolVADyXKF : $@convention(thin) (@guaranteed @noescape @callee_guaranteed () -> Bool) -> Bool
func call_auto_closure(_ x: @autoclosure () -> Bool) -> Bool {
  // CHECK: bb0([[CLOSURE:%.*]] : @guaranteed $@noescape @callee_guaranteed () -> Bool):
  // SEMANTIC SIL TODO: This copy is clearly unnecessary.
...
func test_auto_closure_with_capture(_ x: Bool) -> Bool {
  ...
  // CHECK: [[BORROWED_CVT:%.*]] = begin_borrow [[CVT]]
  // CHECK: [[RET:%.*]] = apply {{%.*}}([[BORROWED_CVT]])
  // CHECK: return [[RET]]
...
func test_auto_closure_without_capture() -> Bool {
  ...
  // CHECK: [[BORROWED_THICK:%.*]] = begin_borrow [[THICK]]
  // CHECK: [[RET:%.*]] = apply {{%.*}}([[BORROWED_THICK]])
  // CHECK: return [[RET]]
...
public class Sub : Base {
  ...
  // CHECK: [[RET:%.*]] = apply [[AUTOCLOSURE_CONSUMER]]([[BORROWED_CVT]])
