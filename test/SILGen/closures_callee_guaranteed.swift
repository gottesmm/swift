...
// CHECK-LABEL: sil @{{.*}}apply{{.*}} : $@convention(thin) (@guaranteed @noescape @callee_guaranteed () -> Int)
...
// CHECK:   [[BORROWED_C3:%.*]] = begin_borrow [[C3]]
// CHECK:   [[A:%.*]] = function_ref @{{.*}}apply{{.*}} : $@convention(thin) (@guaranteed @noescape @callee_guaranteed () -> Int) -> Int
// CHECK:   apply [[A]]([[BORROWED_C3]]) : $@convention(thin) (@guaranteed @noescape @callee_guaranteed () -> Int) -> Int
public func test() {
