...
// CHECK-NEXT: function_ref reabstraction thunk helper from @escaping @callee_guaranteed (@in_guaranteed Swift.Int, @in_guaranteed Swift.Int) -> (@unowned Swift.Bool) to @callee_guaranteed (@unowned Swift.Int, @unowned Swift.Int) -> (@unowned Swift.Bool)
// CHECK-NEXT: [[THUNK:%.*]] = function_ref @$SS2iSbIegnnd_S2iSbIgyyd_TR : $@convention(thin) (Int, Int, @guaranteed @callee_guaranteed (@in_guaranteed Int, @in_guaranteed Int) -> Bool) -> Bool
...
// CHECK: [[THUNK:%.*]] = function_ref @$SS2iSbIgyyd_S2iSbIgnnd_TR : $@convention(thin) (@in_guaranteed Int, @in_guaranteed Int, @guaranteed @noescape @callee_guaranteed (Int, Int) -> Bool) -> Bool
// CHECK-NEXT: [[FN:%.*]] = partial_apply [callee_guaranteed] [[THUNK]]([[CONV_FN]]) : $@convention(thin) (@in_guaranteed Int, @in_guaranteed Int, @guaranteed @noescape @callee_guaranteed (Int, Int) -> Bool) -> Bool
// CHECK-NEXT: [[CONV_FN:%.*]] = convert_function [[FN]]
...
// CHECK:  [[R:%.*]] = function_ref @$S17default_arguments15throwingDefault7closureySbyKXE_tKF : $@convention(thin) (@guaranteed @noescape @callee_guaranteed () -> (Bool, @error Error)) -> @error Error
...
// CHECK:  [[E:%.*]] = convert_function [[C]] : $@callee_guaranteed () -> (Bool, @error Error) to $@noescape @callee_guaranteed () -> (Bool, @error Error)
// CHECK:  [[BORROWED_E:%.*]] = begin_borrow [[E]]
// CHECK:  [[R:%.*]] = function_ref @$S17default_arguments26throwingAutoclosureDefault7closureySbyKXK_tKF : $@convention(thin) (@guaranteed @noescape @callee_guaranteed () -> (Bool, @error Error)) -> @error Error
// CHECK:  try_apply [[R]]([[BORROWED_E]])
...
// CHECK:   [[BORROWED_E:%.*]] = begin_borrow [[E]]
// CHECK:   [[R:%.*]] = function_ref @$S17default_arguments0A3Arg7closureySbyXE_tF : $@convention(thin) (@guaranteed @noescape @callee_guaranteed () -> Bool) -> ()
...
// CHECK:  [[E:%.*]] = convert_function [[C]] : $@callee_guaranteed () -> Bool to $@noescape @callee_guaranteed () -> Bool
// CHECK:  [[BORROWED_E:%.*]] = begin_borrow [[E]]
// CHECK:  [[R:%.*]] = function_ref @$S17default_arguments21autoclosureDefaultArg7closureySbyXK_tF : $@convention(thin) (@guaranteed @noescape @callee_guaranteed () -> Bool) -> ()
// CHECK:  apply [[R]]([[BORROWED_E]])
