...
struct S {
  ...
  init() {
    ...
    // CHECK-NEXT:   [[B_MARKED_SELF_BOX:%.*]] = begin_borrow [[MARKED_SELF_BOX]]
    // CHECK-NEXT:   [[PB:%.*]] = project_box [[B_MARKED_SELF_BOX]]
    ...
    // CHECK-NEXT:   end_borrow [[B_MARKED_SELF_BOX]]
...
enum E {
  ...
  init() {
    ...
    // CHECK:   [[B_MARKED_E_BOX:%.*]] = begin_borrow [[MARKED_E_BOX]]
    // CHECK:   [[PB:%.*]] = project_box [[B_MARKED_E_BOX]]
...
struct S2 {
  ...
  init() {
    ...
    // CHECK:   [[PB:%.*]] = project_box [[B_MARKED_SELF_BOX]]
...
@objc class C2 {
  ...
  convenience init(x: X) {
    ...
    // CHECK:   [[MARKED_SELF_BOX:%[0-9]+]] = mark_uninitialized [delegatingself] [[SELF_BOX]]
    // CHECK:   [[B_MARKED_SELF_BOX:%.*]] = begin_borrow [[MARKED_SELF_BOX]]
    // CHECK:   [[PB_SELF:%.*]] = project_box [[B_MARKED_SELF_BOX]]
