...
// CHECK-NEXT: [[BORROWED_CVT:%.*]] = begin_borrow [[CVT]]
// CHECK:      [[T0:%.*]] = function_ref @$S10reabstract6takeFn{{[_0-9a-zA-Z]*}}F
// CHECK-NEXT: apply [[T0]]<Int>([[BORROWED_CVT]])
// CHECK-NEXT: end_borrow [[BORROWED_CVT]] from [[CVT]]
// CHECK-NEXT: destroy_value [[CVT]]
// CHECK-NEXT: tuple ()
// CHECK-NEXT: return
// CHECK-NEXT: } // end sil function '$S10reabstract5test0yyF'

// CHECK:    sil shared [transparent] [serializable] [reabstraction_thunk] [[THUNK]] : $@convention(thin) (@in_guaranteed Int, @guaranteed @noescape @callee_guaranteed (Int) -> Optional<Int>) -> @out Optional<Int> {
...
// CHECK-LABEL: sil shared [transparent] [serializable] [reabstraction_thunk] @$SypIgn_SiSgIgy_TR : $@convention(thin) (Optional<Int>, @guaranteed @noescape @callee_guaranteed (@in_guaranteed Any) -> ()) -> ()
