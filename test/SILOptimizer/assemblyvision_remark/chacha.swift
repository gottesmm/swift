...
public func run_ChaCha(_ N: Int) {
  ...
  checkResult(checkedtext) // expected-remark {{release of type 'Array<UInt8>'}}
  ...
  for _ in 1...N {
    ...
    print(plaintext.first!) // expected-remark @:11 {{heap allocated ref of type '_ContiguousArrayStorage<Any>'}}
                            // expected-remark @-1:27 {{release of type '_ContiguousArrayStorage<Any>'}}
  }
} // expected-remark {{release of type 'Builtin.BridgeObject'}}
  // expected-note @-7 {{of 'plaintext.rawValue._storage._buffer'}}
  // expected-remark @-2 {{release of type 'Builtin.BridgeObject'}}
  // expected-note @-16 {{of 'nonce'}}
  // expected-remark @-4 {{release of type 'Builtin.BridgeObject'}}
