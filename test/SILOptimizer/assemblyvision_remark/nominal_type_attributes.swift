...
struct Struct {
    func printMe() {
        callPrint(global) // expected-remark {{begin exclusive access to value of type 'String'}}
                          // expected-note @-6 {{of 'global'}}
                          // expected-remark @-2 {{end exclusive access to value of type 'String'}}
                          // expected-note @-8 {{of 'global'}}
                          // expected-remark @-4 {{retain of type 'Builtin.BridgeObject'}}
                          // expected-note @-10 {{of 'global'}}
                          // expected-remark @-6 {{release of type 'String'}}
                          // expected-note @-12 {{of 'global._object._object._guts'}}
    }
}
...
enum Enum {
    func callPrintMe() {
        callPrint(global) // expected-remark {{begin exclusive access to value of type 'String'}}
                          ...
                          // expected-remark @-6 {{release of type 'String'}}
                          // expected-note @-33 {{of 'global._object._object._guts'}}
    }
}
