...
class ReduceRuntimeCrashingCausingPasses(list_reducer.ListReducer):
    ...
    def run_test(self, prefix, suffix):
        ...
        result = self.invoker.invoke_with_passlist(
            ...
            self.invoker.get_suffixed_filename(suffix_hash, ext='.o'))
        ...
        # passes.
        prefix_sib_path = self.invoker.get_suffixed_filename(prefix_hash)
        prefix_obj_path = self.invoker.get_suffixed_filename(prefix_hash, ext='.o')
        result = self.invoker.invoke_with_passlist(
            prefix,
            prefix_sib_path,
            prefix_obj_path)
        ...
        self.invoker.input_file = prefix_sib_path
...
def invoke_runtime_bug_reducer(args):
...
list of passes that the perf pipeline"""
    ...
    if args.pass_list is None:
        ...
        passes = ['-' + x[1] for x in passes if x[1] != 'sil-assume-single-threaded']
    else:
    ...
    compile_invoker = swift_tools.SwiftOptCompileInvoker(config, tools, args.input_file,
                                                         extra_args, args.run_script)

    # Make sure that the base case /does/ crash.
    sib_filename = compile_invoker.get_suffixed_filename('base_case')
    obj_filename = compile_invoker.get_suffixed_filename('base_case', ext=".o")
    result = compile_invoker.invoke_with_passlist(passes, sib_filename, obj_filename)
    ...
    r = ReduceRuntimeCrashingCausingPasses(passes, compile_invoker)
    if not r.reduce_list():
        print("Failed to find miscompiling pass list!")

    cmdline = compile_invoker.cmdline_with_passlist(r.target_list)
    print("*** Found miscompiling passes!")
    print("*** Final File: %s" % compile_invoker.input_file)
    ...
    input_file = compile_invoker.input_file
...
def add_parser_arguments(parser):
    """Add parser arguments for runtime_bug_reducer"""
    parser.set_defaults(func=invoke_runtime_bug_reducer)
    parser.add_argument('input_file', help='The input file to optimize')
    parser.add_argument('run_script', help='')
